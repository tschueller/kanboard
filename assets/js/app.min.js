...
function render(element,html){var containerElement=document.createElement("div");containerElement.id="tooltip-container";containerElement.innerHTML=html;containerElement.addEventListener("mouseleave",onMouseLeaveContainer,!1);containerElement.addEventListener("mouseenter",mouseOnTooltip,!1);containerElement.mouseOnTooltip=!1;containerElement.style.visibility="hidden";document.body.appendChild(containerElement);var elementRect=element.getBoundingClientRect();if(elementRect.left>(window.innerWidth-600)){var right=window.innerWidth-elementRect.right-window.scrollX;containerElement.style.right=right-25+"px"}else{var left=elementRect.left+window.scrollX;containerElement.style.left=left-25+"px"}
var containerRect=containerElement.getBoundingClientRect();var top=elementRect.top+window.scrollY;if(top+containerRect.height-window.scrollY>window.innerHeight){top-=containerRect.height;containerElement.style.top=top+20+"px"}else{containerElement.style.top=top-20+"px"}
containerElement.style.visibility="visible";document.body.onclick=function(event){if(!containerElement.contains(event.target)){destroy()}}}
...
KB.onKey('?',function(){if(!KB.modal.isOpen()){KB.modal.open(KB.find('body').data('keyboardShortcutUrl'),'',!0)}});KB.onKey('Escape',function(){if(!KB.exists('#suggest-menu')){KB.trigger('modal.close');_KB.get("Dropdown").close()}
if(KB.exists('#task-view')){goToLink('a.view-board')}},!0);KB.onKey('Backspace',function(){if(KB.exists('#task-view')){goToLink('a.view-board')}},!0);KB.onKey('Enter',submitForm,!0,!0);KB.onKey('Enter',submitForm,!0,!1,!0);KB.onKey('b',function(){if(!KB.modal.isOpen()){KB.trigger('board.selector.open')}});if(KB.exists('#board')){KB.onKey('c',function(){if(!KB.modal.isOpen()){_KB.get('BoardHorizontalScrolling').toggle()}});KB.onKey('s',function(){if(!KB.modal.isOpen()){_KB.get('BoardCollapsedMode').toggle()}});KB.onKey('n',function(){if(!KB.modal.isOpen()){KB.modal.open(KB.find('#board').data('taskCreationUrl'),'large',!1)}})}
