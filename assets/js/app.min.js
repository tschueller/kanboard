...
this.render=function(){KB.on('modal.submit',onSubmit);KB.on('modal.stop',function(){KB.removeListener('modal.submit',onSubmit)});KB.on('modal.close',function(){files=[];KB.removeListener('modal.submit',onSubmit)});inputElement=KB.dom('input').attr('type','hidden').attr('name','screenshot').build();inputFileElement=buildFileInputElement();dropzoneElement=buildDropzoneElement();containerElement.appendChild(inputElement);containerElement.appendChild(inputFileElement);containerElement.appendChild(dropzoneElement);initialize();onLoad()};function onSubmit(){var form=document.querySelector('#modal-content form');if(form){var url=form.getAttribute('action');if(url){KB.http.postForm(url,form).success(function(response){KB.trigger('modal.stop');if(response){KB.modal.replaceHtml(response)}else{KB.modal.close()}}).error(function(response){KB.trigger('modal.stop');if(response.hasOwnProperty('message')){window.alert(response.message)}})}}}});KB.component('file-upload',function(containerElement,options){var inputFileElement=null;var dropzoneElement=null;var files=[];var currentFileIndex=0;function onProgress(e){if(e.lengthComputable){var progress=e.loaded/e.total;var percentage=Math.floor(progress*100);KB.find('#file-progress-'+currentFileIndex).attr('value',progress);KB.find('#file-percentage-'+currentFileIndex).replaceText('('+percentage+'%)')}}
